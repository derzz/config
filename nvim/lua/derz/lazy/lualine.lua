...
local icons = require('derz.local.icons')
...
spec.opts.options = {
  ...
  section_separators = { left = '', right = '' },
    ...
    disabled_filetypes = {'alpha', 'NvimTree', 'packer'}
}
...
spec.opts.sections.lualine_c = {
  { '%=', padding = 0 },
  {
    ...
    padding = 0,
    color = function()
      local mode = require('derz.local.theme').get_mode()
...
function spec.config(_, opts)
  ...
  opts = vim.tbl_deep_extend('force', opts, {
    options = {
      theme = require('derz.local.theme').gen_lualine_theme('auto'),
    },
  })
